...
const string connectionString = "Server=localhost;Database=PlayGroundGF;User id=sa;password=yourStrong(!)Password;TrustServerCertificate=true";
var optionsBuilder = new DbContextOptionsBuilder<ApplicationDbContext>();
optionsBuilder.UseSqlServer(connectionString);

var channelVisibilityProvider = new ChannelVisibilityProvider { VisibleChannels = new List<int> {1,2}};
ApplicationDbContext context;
using (context = new ApplicationDbContext(optionsBuilder.Options, channelVisibilityProvider))
{
   ...
   Console.WriteLine("There are {0} stores in channel 2\n", context.Stores.Count(s => s.ChannelId == 2));
    ...
    channelVisibilityProvider.VisibleChannels = new List<int> {1};
    Console.WriteLine("There are {0} stores", context.Stores.Count());
    Console.WriteLine("There are {0} stores in channel 1", context.Stores.Count(s=> s.ChannelId==1));
    Console.WriteLine("There are {0} stores in channel 2\n", context.Stores.Count(s=> s.ChannelId==2));
}
...
var dynamicChannelVisibilityProvider = new QueryChannelVisibilityProvider(new ApplicationDbContext(optionsBuilder.Options));
var otherContext = new ApplicationDbContext(optionsBuilder.Options, dynamicChannelVisibilityProvider);

Console.WriteLine("There are {0} stores", otherContext.Stores.Count());
Console.WriteLine("There are {0} stores in channel 1", otherContext.Stores.Count(s => s.ChannelId == 1));
Console.WriteLine("There are {0} stores in channel 2", otherContext.Stores.Count(s => s.ChannelId == 2));
return;
